...
typealias GameCountClosure = (Bool?) -> Void
...
//        checkForNewGames(count: currentGameCount, completionHandler: {status in
//            if status! {
//                self.pullNewGames()
//            }
//        })
        ...
        pullGameDictionary() { gameDictionary in
            
            finalGameDictionary = gameDictionary as! Dictionary<String, String>
//            let userDefaults = UserDefaults.standard
//            var decodedGamesList = [Game]()
//            
//            if let decoded = userDefaults.object(forKey: "sortedGameList") as? Data {
//                decodedGamesList = NSKeyedUnarchiver.unarchiveObject(with: decoded) as! [Game]
//                for game in decodedGamesList {
...
//            }
            ...
            for (title, imagePath) in finalGameDictionary {
                print("flag 1")
                let imageRef = storage.child(imagePath)
                
                self.pullGameImage(reference: imageRef) { gameImage in
                    self.gameArray.append(Game(_title: title, _image: gameImage!, _added: false))
        ...
        print("Should be finished appending")
    ...
    func checkForNewGames(count: Int, completionHandler: @escaping (GameCountClosure)) {
        var status = false
